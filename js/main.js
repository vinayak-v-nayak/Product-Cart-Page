...
sliderImgSelector.forEach(el => {
    el.addEventListener("click", () => {
        sliderImg.src = `/images/image-product-${el.getAttribute("value")}.webp`;
    });
});
...
const previousImg = (img) => {
    if (counter > 1) {
        counter--;
        img.src = `/images/image-product-${counter}.webp`;
    } else {
        counter = 4;
        img.src = `/images/image-product-${counter}.webp`;
    }
...
sliderFocusImgSelector.forEach(el => {
    el.addEventListener("click", () => {
        sliderImgFocus.src = `/images/image-product-${el.getAttribute("focus-value")}.webp`;
        counter = el.id;
...
const updateCart = () => {
    ...
    let htmlCode = 
        `<img class="cart__product--img" src="./images/image-product-1-thumbnail.webp" alt="">
